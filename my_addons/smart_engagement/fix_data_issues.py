...
def fix_basic_data_issues(env):
    ...
    try:
        report_model = env['smart_engagement.top_products_by_category_report']
        report_model.init()
        env.cr.commit()
        print("   Top products view recreated successfully")
        
        # Test if view has data
        env.cr.execute("SELECT COUNT(*) FROM smart_engagement_top_products_by_category_report")
...
2. In the shell, run:
   exec(open('/mnt/c/Users/anton/labs/odoo/odoo/my_addons/smart_engagement/fix_data_issues.py').read())
