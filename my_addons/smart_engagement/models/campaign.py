...
class Campaign(models.Model):
    _name = "smart_engagement.campaign"
    ...
    customer_segmentation_id = fields.Many2one(
        "smart_engagement.customer_segmentation",
...
""",
        ...
        for rec in self:
            ...
            cron_vals = {
                ...
                "model_id": self.env.ref(
                    "smart_engagement.model_smart_engagement_campaign"
                ).id,
                "state": "code",
            ...
            for cron in campaign_crons:
                try:
                    ...
                    # For recurring jobs, check if the associated campaign is no longer running
                        ...
                        campaign = self.env["smart_engagement.campaign"].search(
    ...
    def _create_api_log(
        ...
        """
        try:
            self.env["smart_engagement.whatsapp_api_log"].create(
                {
                    "message": message,
                    "status": status,
                    "at": execution_time,
